...
class M_Santri extends Model
{
    ...
    public function getAbsensiData($idSantri)
    {
        ...
        $absensi = M_Absensi::where('id_santri', $idSantri)
            ->where('tanggal', $tanggal)
            ->where('bulan', $bulan)
            ->where('tahun', $tahun)
            ->first();

        // Kalau ada data, kembalikan statusnya (0, 1, atau 2)
        // Kalau tidak ada data, kembalikan null
        return $absensi ? $absensi->active : null;
    }
