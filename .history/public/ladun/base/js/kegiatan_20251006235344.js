...
var appKegiatan = new Vue({
    ...
    axios.get(`${server}app/kegiatan/${id}`).then((response) => {
        ...
        if (data.image && data.image !== "") {
            imgPreview.src = server + "storage/" + data.image; // pastikan storage sudah symlink ke public
        } else {
            imgPreview.src = "https://via.placeholder.com/200x150?text=No+Image"; // placeholder jika tidak ada
        }

        // Reset input file agar tidak ada file lama tersimpan
...
function previewImage(context) {
    ...
    if (file) {
        ...
        // jangan kosongkan preview lama saat edit
