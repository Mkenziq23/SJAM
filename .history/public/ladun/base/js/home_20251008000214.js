...
const routeRatingsStore = "{{ route('ratings.store') }}";

// Klik bintang → aktifkan warna
stars.forEach((star, index) => {
    star.addEventListener("click", function () {
        const value = this.getAttribute("data-value");
        ratingValue.value = value; // simpan ke input hidden

        // Reset semua bintang ke outline
        stars.forEach((s, i) => {
...
// Kirim form ke server via Axios
        ...
        axios.post("/ratings", formData)
            .then((res) => {
                Swal.fire({
                    icon: "success",
                    title: "Berhasil!",
                    text: res.data.message || "Terima kasih atas penilaian Anda!",
                    timer: 2000,
