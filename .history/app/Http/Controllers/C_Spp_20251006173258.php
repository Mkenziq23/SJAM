...
{
    $spp = M_Spp::with(['santriData', 'pengurusData'])->where('token_pembayaran', $token)->first();

    if (!$spp) {
        abort(404, 'Data tidak ditemukan');
    }

    // Ambil alamat dan email dari tbl_setting
    $alamat = M_Setting::where('nama_setting', 'alamat')->value('value');
    $email  = M_Setting::where('nama_setting', 'email')->value('value');
