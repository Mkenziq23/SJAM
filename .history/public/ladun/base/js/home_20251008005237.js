...
function checkDataSantriAtc() {
    ...
    if (nHandphone.length === 0) {
        pesanUmumApp("warning", "Oops!", "Harap masukkan nomor handphone terlebih dahulu.");
        return;
    }

    if (!statusBtnSetor) {
        pesanUmumApp("info", "Tunggu sebentar...", "Proses pengecekan sedang berlangsung.");
        return;
    }
    ...
    let ds = { hp: nHandphone };
    axios.post(rCheckDataSantri, ds)
        .then(function (res) {
            Swal.close();
            if (res.data.success === true) {
                // ✅ Data santri ditemukan
                let santri = res.data;
                let statusBayar = santri.status_bayar; // pastikan backend kirim ini
                ...
                if (statusBayar === "sudah") {
                    ...
                    Swal.fire({
                        icon: "success",
                        title: "Sudah Bayar 🎉",
                        text: `Santri ${santri.nama_santri} sudah melakukan pembayaran bulan ini.`,
                    ...
                    Swal.fire({
                        ...
                        title: "Belum Bayar 😔",
                        html: `
                            <p>Santri <strong>${santri.nama_santri}</strong> belum melakukan pembayaran bulan ini.</p>
                            <p>Silakan unggah bukti pembayaran dengan menekan tombol di bawah.</p>
                        `,
                        ...
                        if (result.isConfirmed) {
                            ...
                            // isi data santri ke form
                            document.querySelector("#rIdSantri").innerHTML = santri.id_santri;
                            document.querySelector("#rNamaSantri").innerHTML = santri.nama_santri;
                            document.querySelector("#rNamaOrangTua").innerHTML = santri.nama_orang_tua;
                            document.querySelector("#rKelas").innerHTML = santri.kelas;
                ...
                pesanUmumApp("error", "Tidak Ditemukan", "Data santri tidak ditemukan. Periksa kembali nomor handphone.");
                $("#divBothSantriData").hide();
            }
        })
        .catch(function (err) {
            Swal.close();
            pesanUmumApp("error", "Kesalahan", "Terjadi kesalahan saat menghubungi server.");
        })
        .finally(() => {
            statusBtnSetor = true;
        });
