...
document.addEventListener("DOMContentLoaded", function () {
  ...
  stars.forEach(star => {
    ...
    star.addEventListener("mouseout", () => highlightStars(ratingValue.value));
  ...
  form.addEventListener("submit", async (e) => {
    ...
    if (!ratingValue.value) {
      Swal.fire("Oops!", "Silakan pilih jumlah bintang terlebih dahulu.", "warning");
      return;
    }

    const formData = {
      name: document.getElementById("ratingName").value,
      reason: document.getElementById("ratingReason").value,
    ...
    try {
      ...
      Swal.fire("Berhasil!", data.message ?? "Penilaian berhasil dikirim.", "success");
      form.reset();
      highlightStars(0);
      ratingValue.value = "";
